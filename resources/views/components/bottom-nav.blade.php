...
<nav {{ $attributes->merge(['class' => 'fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] bg-white/95 backdrop-blur z-40 border-t border-slate-100']) }}
    ...
    <div class="grid grid-cols-5 items-end px-2 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] relative">
        ...
        <div class="flex justify-center">
            <a href="{{ route('catalog') }}"
               class="absolute -top-5 flex items-center justify-center w-14 h-14 rounded-full grad-primary text-white shadow-lg ring-4 ring-white active:scale-95 transition">
